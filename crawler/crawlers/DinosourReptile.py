        ...
        try:
            ...
            for row in rows:
                ...
                if days_left is not None:
                    ...
                    dino_events.append({
                        "event_name": "공룡 파충류 박람회",
                        "location": loc,
                        "event_date": event_date_str,
                        "full_title": title,
                        "link": clean_link,
                        "image_url": None # 2단계를 위해 비워둠
                    })

            # ==========================================
            # 📸 2단계: 본문 접속 및 포스터 이미지 추출 (크기 기준)
            # ==========================================
            print(f"\n📸 [공룡 파충류 박람회] 2단계: 포스터 이미지 추출 시작 (총 {len(dino_events)}건)...")
            DEFAULT_IMAGE_URL = "https://via.placeholder.com/400x300?text=No+Image"

            for ev in dino_events:
            ...
            print("\n🦖🦕 [공룡 파충류 박람회 확정 리스트] 🦕🦖")
            print("="*60)
            for i, ev in enumerate(dino_events, 1):
                print(f"[{i}] 🎯 행사 예측 및 포스터 추출 완료!")
